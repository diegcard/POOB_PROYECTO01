...
public class SpiderWebContest{
    ...
    private static Node[] seg = new Node[N * 4];
    // n es el número de hilos, s es el hilo favorito de la araña
    private int s;
    private int n;
    private String[] colors = {"salmon", "lilac", "turquoise", "brown", "purple", "lightgray", "pink",
    ...
    // modigica el arbol de segmentos con los piuentes 
    ...
    private boolean determinateThreadStart(int initStrand, ArrayList<int[]> bridges, int distance){
        if(distance == seg[initStrand].min_inc){
            return true;
        }else{
            return false;
        }
    ...
    //hacer el arreglo pasoApaso para saber sobre que strands iterar en el proceso de añadir en Dec
    private ArrayList<Integer> pasoAPasocreatePasoAPasoDec(int initStrand, int strands){
        ArrayList<Integer> pasoAPaso = new ArrayList<Integer>(); 
    ...
    private ArrayList<int[]> addBridgeWithDeterminateThreadStart(int initStrand, ArrayList<int[]> bridges,int strands){
        ArrayList<int[]> newBridges = new ArrayList<int[]>();
